...
SoftwareSerial G1(10, 11); //TX,RX
SoftwareSerial G2(8,9);
int pos=90;
char BluetoothData; 
int servo=6;
int angle=90;
...
void loop()
{
  ...
  servoPulse(servo, pos);
  G2.listen();
  BluetoothData=G2.read();

  
  if(BluetoothData=='1'){ pos=pos+10; delay(100);
   }

   
  if (BluetoothData=='0'){ pos=90; delay(100);
  }

  if (BluetoothData=='2'){ pos= pos-10; delay(100);
  }
...
//  float accel_vector_length = sqrt(pow(accel_x,2) + pow(accel_y,2) + pow(accel_z,2));
  ...
  G1.print(F("DEL:"));              //Delta T
  G1.print(dt, DEC);
  G1.print(F("#ACC:"));              //Accelerometer angle
  G1.print(accel_angle_x, 2);
  G1.print(F(","));
  G1.print(accel_angle_y, 2);
  G1.print(F(","));
  G1.print(accel_angle_z, 2);
  G1.print(F("#GYR:"));
  G1.print(unfiltered_gyro_angle_x, 2);        //Gyroscope angle
  G1.print(F(","));
  G1.print(unfiltered_gyro_angle_y, 2);
  G1.print(F(","));
  G1.print(unfiltered_gyro_angle_z, 2);
  G1.print(F("#FIL:"));             //Filtered angle
  G1.print(angle_x, 2);
  G1.print(F(","));
  G1.print(angle_y, 2);
  G1.print(F(","));
